...
import { MBTIResult, TagSort } from '@/components/report/MBTI/type';
 ...
 */
const LookReport = () => {
    const params = useParams() as { userId: string, people: string }
    const [resultDetial, setResultDetial] = useState<any>();
        ...
        <div className={styles.detail_layout}>
            <header>
                <Breadcrumb separator=">" className={styles.detail_nav}>
                    <Breadcrumb.Item href="#/evaluation/management">人才报告</Breadcrumb.Item>
                    <Breadcrumb.Item href={`#/evaluation/peopleReport/detail/${params.userId}`}>{resultDetial?.user?.name}</Breadcrumb.Item>
